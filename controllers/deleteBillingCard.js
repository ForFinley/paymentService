...
module.exports.handler = async function(req, res) {
  ...
  if (!stripeCustomerId && !stripeCardId) {
    return res
      .status(500)
      .send({ message: "ERROR : User has no card on file, can't delete." });
  }

  try {
    const result = await stripe.customers.deleteSource(
      req.user.stripeCustomerId,
      req.user.stripeCardId
    );
    console.log(result);
    ...
    let updateRequest = {
      ...
      UpdateExpression:
        'remove #stripeBillingCardBrand, #stripeBillingCardExp,#stripeBillingCardLast4, #stripeCardId',
    ...
    return res.status(200).send({ message: 'SUCCESS : Card deleted.' });
  } catch (err) {
    console.log(
      '**ERROR** A problem occurred while attempting to delete card.**',
      err
    );
    return res.status(500).send({
      message: 'ERROR :  A problem occurred while attempting to delete card.'
    });
  }
};
